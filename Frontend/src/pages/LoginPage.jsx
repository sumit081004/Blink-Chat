...
import log from '../assets/log3.jpeg'
...
const Loginpage = () => {
  ...
  const onSubmitHandler = (event)=>{
    ...
    if (!agreeTerms) {
    toast.error(" Please agree to the terms of user  & privacy policy");
    return;
    }
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      {/*----left-----*/}
      <img src={log} alt="" className='rounded-xl w-[min(30vw,250px)]'/>
...

      {
        currState ==="Sign Up" && isDataSubmitted && (
          ...
          row={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500' placeholder='provide a sort bio...' required></textarea>
      )
      }
       ...
       <div className='flex flex-col gap-2'>
        {currState === "Sign Up" ? (
          <p className='text-sm text-gray-600'>Already have an account? <span onClick={()=>{setCurrState("Login"); setIsDataSubmitted(false)}} className='font-medium text-violet-600 cursor-pointer'>Login here</span></p>
        ) : (
          <p className='text-sm text-gray-600'>Create an account <span 
          onClick={()=> setCurrState("Sign Up")} className='font-medium 
          text-violet-600 cursor-pointer'>Click here</span></p>
        )}
